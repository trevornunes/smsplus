...
{
	return "shared/misc/roms/sms/game.sms";
}
...
#else
        // cycle instead i.e we can keep on hitting F1 and it will cycle through FX
        if( sdl_video.current_filter != -1) {
            if( sdl_video.current_filter++ >= FILTER_NUM)
            	sdl_video.current_filter=0;

            if( sdl_video.current_filter )
            {
            	SDL_FillRect( sdl_video.surf_filter, NULL,0);
            }
        }
...
int sdlsms_init(const t_config* pcfg)
{
  ...
  if( rombrowser_rom_count() < 1) {
	  printf("ERROR: no roms!");
      return 0;
  }
  rom_filename = rombrowser_next();
#else
...
#else
  printf("QNX: skipping joystick init\n");
#endif
